...
class DockerJenkinsServer:
    ...
    def _image_build(self) -> str:
        ...
        try:
            ...
            if ': 1' in str(error):
                ...
                print2(
                    '   Possible Reason: Unknown command, misspelling, or end of line sequence (CRLF/LF)',
                    bold=True,
                    color='red',
                )
            elif ': 2' in str(error):
                ...
                print2(
                    '   Possible Reason: Script or command does not have sufficient privileges', bold=True, color='red'
                )
            else:
                print2(
                    '   Please see error code: https://www.thegeekstuff.com/2010/10/linux-error-codes/',
                    bold=True,
                    color='red',
                )
            return ''
    ...
    def _volumes_create(self) -> list:
        ...
        mount = docker.types.Mount(
            target='/var/run/docker.sock', source='/var/run/docker.sock', type='bind', read_only=False
        )
        self.volumes_mounts.append(mount)
